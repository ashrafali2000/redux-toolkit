...
const HomePage = () => {
    ...
    return (
      ...
      >
        <div className="flex justify-center items-center gap-2">
          <svg
            aria-hidden="true"
            className="inline w-10 h-10 md:w-16 md:h-16 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600"
            viewBox="0 0 100 101"
            fill="none"
  ...
  const myProducts = JSON.stringify(data);
  const myNewProducts = JSON.parse(myProducts);
  const myUsers = JSON.stringify(users);
  const myNewUsers = JSON.parse(myUsers);
  const myAllUsers = myNewUsers.data;
